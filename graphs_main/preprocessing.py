...
def preprocess_data(data_dir, train_rating, mode='graph', num_train_subgraphs=25, num_test_subgraphs=34):
    ...
    train_data = filter_users_by_movie_count(train_data, min_movie_count=20)
    test_data = filter_users_by_movie_count(test_data, min_movie_count=20)
...
def filter_users_by_movie_count(data, min_movie_count=20):
    # Count the number of movies each user has rated
    user_movie_counts = data.groupby('user').size()
    users_with_few_movies = user_movie_counts[user_movie_counts < min_movie_count].index
    filtered_data = data[data['user'].isin(users_with_few_movies)]
